...
from drheader.utils import get_rules_from_uri
...
@click.option(
    ...
    "--verify-enable",
    default=True,
    help="Bool indicating SSL verification",
)
...
@click.option(
    "--json", "json_output", help="Output report as json", is_flag=True
)
@click.option("--debug", help="Show error messages", is_flag=True)
@click.option(
    "--rules", "rule_file", help="Use custom rule set", type=click.File()
)
@click.option(
    "--rules-uri", "rule_uri", help="Use custom rule set, downloaded from URI"
)
@click.option(
    "--merge",
    help="Merge custom file rules, on top of default rules",
    is_flag=True,
)
@click.option(
    "--cross-origin-isolated",
    help="Enable cross-origin isolation validations",
    is_flag=True,
)
def compare(
    file, json_output, debug, rule_file, rule_uri, merge, cross_origin_isolated
):
    """
    ...
    for i in data:
        ...
        audit.append(
            {"url": i["url"], "report": drheader_instance.reporter.report}
        )
...
@click.option(
    "--junit",
    help="Produces a junit report with the result of the scan",
    is_flag=True,
)
@click.option(
    "--cross-origin-isolated",
...
def single(
    ...
    if json_output:
        ...
        if not drheader_instance.reporter.report:
            ...
            click.echo(
                "{0} issues found".format(
                    len(drheader_instance.reporter.report)
                )
            )
...
def bulk(
    ...
    for i, v in enumerate(urls):
        ...
        drheader_instance.analyze(rules, bool(cross_origin_isolated))
        audit.append(
            {"url": v["url"], "report": drheader_instance.reporter.report}
        )
        if drheader_instance.reporter.report:
            exit_code = EXIT_CODE_FAILURE
