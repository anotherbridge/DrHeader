...
with open(
    os.path.join(os.path.dirname(__file__), "resources/delimiters.json")
) as delimiters:
    _DELIMITERS = utils.translate_to_case_insensitive_dict(
        json.load(delimiters)
    )
...
class Drheader:
    ...
    def __init__(
        ...
        if not headers:
            if not url:
                raise ValueError(
                    "Nothing provided for analysis. Either 'headers' or 'url'"
                    " must be defined"
                )
            else:
                headers = _get_headers_from_url(
    ...
    def analyze(self, rules=None, cross_origin_isolated=False):
        ...
        """
        if not rules:
            rules = utils.translate_to_case_insensitive_dict(
                utils.load_rules()
            )
        else:
        ...
        for header, config in rules.items():
            if (
                header.lower() in _CROSS_ORIGIN_HEADERS
                and not cross_origin_isolated
            ):
                continue
            else:
    ...
    def _analyze_directives(self, config, validator, header):
        for directive, config in config["directives"].items():
            self._validate_rules(
                config, validator, header, directive=directive
            )

    def _analyze_cookies(self, config, validator):
        for cookie, config in config["cookies"].items():
            self._validate_rules(
                config, validator, header="Set-Cookie", cookie=cookie
            )

    def _validate_rules(
        self, config, validator, header, directive=None, cookie=None
    ):
        ...
        if is_required == "false":
            report_item = validator.validate_not_exists(
                config, header, directive=directive, cookie=cookie
            )
            self._add_to_report_if_exists(report_item)
        else:
            exists = self._validate_exists(
                is_required, config, validator, header, directive, cookie
            )
            if exists:
                self._validate_enforced_value(
                    config, validator, header, directive
                )
                self._validate_avoid_and_contain_values(
                    config, validator, header, directive, cookie
                )

    def _validate_exists(
        self, is_required, config, validator, header, directive, cookie
    ):
        if is_required == "true":
            report_item = validator.validate_exists(
                config, header, directive=directive, cookie=cookie
            )
            self._add_to_report_if_exists(report_item)
            ...
            return directive in utils.parse_policy(
    ...
    def _validate_enforced_value(self, config, validator, header, directive):
        if "value" in config:
            report_item = validator.validate_value(
                config, header, directive=directive
            )
            self._add_to_report_if_exists(report_item)
        elif "value-any-of" in config:
            report_item = validator.validate_value_any_of(
                config, header, directive=directive
            )
            self._add_to_report_if_exists(report_item)
        elif "value-one-of" in config:
            report_item = validator.validate_value_one_of(
                config, header, directive=directive
            )
            self._add_to_report_if_exists(report_item)

    def _validate_avoid_and_contain_values(
        self, config, validator, header, directive, cookie
    ):
        if "must-avoid" in config:
            report_item = validator.validate_must_avoid(
                config, header, directive=directive, cookie=cookie
            )
            self._add_to_report_if_exists(report_item)
        if "must-contain" in config:
            report_item = validator.validate_must_contain(
                config, header, directive=directive, cookie=cookie
            )
            self._add_to_report_if_exists(report_item)
        if "must-contain-one" in config:
            report_item = validator.validate_must_contain_one(
                config, header, directive=directive, cookie=cookie
            )
            self._add_to_report_if_exists(report_item)
...
def _get_headers_from_url(url, method, params, headers, verify):
    if not validators.url(url):
        raise ValueError(
            f"Cannot retrieve headers from '{url}'. The URL is malformed"
        )
    ...
    if len(response.raw.headers.getlist("Set-Cookie")) > 0:
        response_headers["set-cookie"] = response.raw.headers.getlist(
            "Set-Cookie"
        )
