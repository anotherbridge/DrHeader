...
class HeaderValidator(base.ValidatorBase):
    ...
    def validate_exists(self, config, header, directive=None, cookie=None):
        ...
        if header not in self.headers:
            ...
            if "value" in config:
                ...
                return report.ReportItem(
                    severity, error_type, header, expected=expected
                )
            else:
    ...
    def validate_value(self, config, header, directive=None):
        ...
        strip_chars = (
            base.get_delimiter(config, "strip")
            if header.lower() in _STRIP_HEADERS
            else None
        )
        header_items = utils.parse_policy(
            header_value, item_delimiter=delimiter, strip=strip_chars
        )
    ...
    def validate_value_one_of(self, config, header, directive=None):
        ...
        if header_value.strip(strip_chars).lower() not in {
            item.lower() for item in accepted
        }:
            severity = config.get("severity", "high")
    ...
    def validate_must_avoid(self, config, header, directive=None, cookie=None):
        ...
        header_value = (
            self._get_cookie(cookie) if cookie else self.headers[header]
        )
        header_items = utils.parse_policy(
            header_value, **config.get("delimiters", {}), keys_only=True
        )
    ...
    def _validate_must_avoid_for_policy_header(self, config, header):
            ...
            if avoid.lower() in header_items:
                ...
                if not non_compliant_directives:
                    ...
                    for ncd in non_compliant_directives:
                        ...
                        ncd_item = {
                            "value": value,
                            "anomalies": ncd_items.get(directive, {}).get(
                                "anomalies", []
                            ) + [avoid],
                        }
        ...
        if ncd_items:
            for directive in ncd_items:
                value, anomalies = (
                    ncd_items[directive]["value"],
                    ncd_items[directive]["anomalies"],
                )
                item = report.ReportItem(
                    severity,
                    error_type,
