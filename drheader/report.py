...
class ErrorType(enum.Enum):
    ...
    CONTAIN_ONE = (
        "Must-Contain-One directive missed. At least one of the expected items"
        " was expected"
    )
    ...
    VALUE_ANY = (
        "Value does not match security policy. At least one of the expected"
        " items was expected"
    )
    VALUE_ONE = (
        "Value does not match security policy. Exactly one of the expected"
        " items was expected"
    )
